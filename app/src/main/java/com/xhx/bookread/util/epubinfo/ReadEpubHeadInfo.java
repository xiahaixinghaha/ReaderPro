package com.xhx.bookread.util.epubinfo;
...
public class ReadEpubHeadInfo {
    ...
    public static BookModel getePubBook(String ePubPath) {
        ...
        if (TextUtils.isEmpty(ePubPath))
            return null;
        //是否是epub书籍
        if (!ePubPath.endsWith(FileTypeUtil.TYPE_EPUB)) {
            return null;
        }
        BookModel book = new BookModel();
        //思路是将指定部分文件解压至指定目录，上一次如果解析过会有残留，当然也可以不清。
        if (MFileUtils.isDir(SAVE_INFO_PATH)) {
            MFileUtils.deleteDir(SAVE_INFO_PATH);
        }
        try {
            //存储content.opf文件路径信息
            ...
            if (ZipUtils.zipSpecifiedFile(ePubPath, SAVE_INFO_PATH, contentOpfPath)) {
                ...
                if (imgXmlFlag != null) {
                    ...
                    String saveImagePath = SAVE_IMAGE_PATH + MFileUtils.delFileSuffix(sourceBookName[sourceBookName.length - 1]) + "/";
                    //4.根据路径解压图片
